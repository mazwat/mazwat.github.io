...
THere is another type of access modifier that is not covered here. A **Protected** field can be accessed only by code in the same `class`, or in a `class` that is derived from that `class`.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTAzNjkyNjU1MiwtNTIyNTUxMzI0LDIwMT
QxNTI4NjFdfQ==
-->